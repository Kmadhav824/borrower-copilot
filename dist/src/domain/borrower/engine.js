...
function affordability(profile, request, rate, product, rules) {
    ...
    if (!income || !expenses || !existing || !upcoming) {
        ...
        if (!upcoming)
            reasons.push(reason("SAFE_UPCOMING_OBLIGATIONS_UNKNOWN", "affordability", "blocking", "SAFE_DEBT_SERVICE_CAP", "A safe borrowing amount cannot be calculated while upcoming monthly obligations are unknown; they are not assumed to be zero.", ["financial.upcomingMonthlyObligations"]));
