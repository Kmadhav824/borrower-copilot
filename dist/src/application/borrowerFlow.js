...
const answered = (answers, id) => answers[id] !== undefined;
...
function debtIsRelevant(answers) {
    ...
    if (existing.kind === "unknown")
        return true;
    const range = toRange(existing);
    return range !== null && range.high > 0;
}
function businessCollateralIsRelevant(answers) {
...
function createBorrowerProfile(answers, incomeType) {
    const credit = isCreditScoreAnswer(answers.creditProfile) ? answers.creditProfile : { status: "unknown", score: { kind: "unknown", reason: "Not provided" } };
...
function createLoanRequest(answers, purpose, terms) {
    return {
