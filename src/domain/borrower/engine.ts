...
import { DEFAULT_RULES, type EngineRules } from "../../rules/config.js";
import type {
  APRResult, AssessmentResult, BorrowerProfile, BorrowingVerdict, ConfidenceLevel,
  EligibilityResult, ExplanationMetadata, FinancialInputs, LoanOfferInputs, LoanRequest,
  MonetaryRange, PercentageRange, ProductType, RateBand, StressResult, TenureOption
} from "./types.js";
...
type ProfileQuality = RateBand["profile"];
type NumberRange = { readonly low: number; readonly high: number };

const INR = (value: NumberRange): MonetaryRange => ({ ...value, currency: "INR" });
const hasKnownTrue = (value: boolean | "unknown"): boolean => value === true;

function stableIncome(profile: BorrowerProfile, financial: FinancialInputs, rules: EngineRules): NumberRange | null {
...
function profileQuality(profile: BorrowerProfile): ProfileQuality {
  ...
  if (profile.credit.scoreStatus === "known" && score && score.low >= 750 && stableAndDocumented) return "strong";
  if (profile.credit.scoreStatus === "unknown" || profile.credit.scoreStatus === "noHistory" || profile.incomeStability === "unknown") return "limited";
  return "standard";
}

function calculateFairRate(product: ProductType, profile: BorrowerProfile, rules: EngineRules): { band: RateBand; reasons: ExplanationMetadata[] } {
  ...
  const reasons: ExplanationMetadata[] = [reason(
    "RATE_PROFILE_BAND", "rate", quality === "stressed" ? "caution" : "info", rules.profileRateBands.id,
    `The fair-rate band reflects a ${quality} evidence and repayment profile.`, ["credit", "incomeStability", "repaymentHistory"]
  )];
  if (profile.credit.scoreStatus !== "known") reasons.push(reason(
    "RATE_UNKNOWN_CREDIT", "rate", "caution", "UNKNOWN_CREDIT_WIDENS_RANGE",
    "Your rate range remains broad because credit-score information is unknown or unavailable; it is not treated as a poor score.", ["credit.score"]
  ));
...
function affordability(
  ...
  product: ProductType,
  rules: EngineRules
): { result: AssessmentResult["affordability"]; reasons: ExplanationMetadata[] } {
  ...
  const safeEmiHigh = Math.max(0, Math.min(
    income.high * cap - existing.low,
    income.high - expenses.low - existing.low - income.high * bufferPercent - upcomingRange.low
  ));
  const tenure = selectedTenure(request, product, rules);
  const safeBorrowing = INR(range(
    principalFromEmi(safeEmiLow, rate.high, tenure),
    principalFromEmi(safeEmiHigh, rate.low, tenure)
  ));
  const safeEmi = INR(range(safeEmiLow, safeEmiHigh));
  reasons.push(reason("SAFE_EMI_EXPLAINED", "emi", safeEmiLow === 0 ? "blocking" : "info", rules.safeDebtServiceCaps.id,
    `Your conservative new-EMI ceiling is based on stable income, essential expenses, existing EMIs, and a retained monthly buffer.`,
    ["financial.monthlyIncome", "financial.essentialMonthlyExpenses", "financial.existingEmis"]
  ));
...
function eligibility(
  ...
  const reasons: ExplanationMetadata[] = [];
  if (!income || !existing) {
    reasons.push(reason("SANCTION_INPUTS_UNKNOWN", "eligibility", "caution", rules.lenderDebtServiceCaps.id,
      "A lender-sanction estimate needs income and existing-EMI information; unknown values are not assumed to be zero.", ["financial.monthlyIncome", "financial.existingEmis"]));
...
function calculateApr(
  request: LoanRequest,
  offer: LoanOfferInputs | undefined,
  rate: RateBand,
  product: ProductType,
  rules: EngineRules
): APRResult {
  ...
  for (const scenario of scenarios) {
    ...
    if (irr === null) return { status: "unavailable", apr: null, totalRepayment: null, allInCost: null, netDisbursed: null, reasons: [reason("APR_INVALID_CASHFLOW", "apr", "blocking", "APR_IRR", "APR could not be calculated because known fees exceed or invalidate net disbursal.", ["offer"])] };
    aprs.push(annualiseMonthlyRate(irr));
    repayments.push(emi * tenure);
    costs.push(emi * tenure - net);
    disbursed.push(net);
  }
  return { status: "estimated", apr: range(aprs[0]!, aprs[1]!) as PercentageRange, totalRepayment: INR(range(repayments[0]!, repayments[1]!)), allInCost: INR(range(costs[0]!, costs[1]!)), netDisbursed: INR(range(disbursed[0]!, disbursed[1]!)), reasons: [reason("APR_IRR", "apr", "info", "APR_IRR", "APR is estimated from net disbursal and scheduled repayments, including known mandatory fees.", ["request.requestedAmount", "offer"])] };
}

function stressTest(profile: BorrowerProfile, request: LoanRequest, rate: RateBand, product: ProductType, safeEmi: number | null, rules: EngineRules): StressResult {
  const income = stableIncome(profile, profile.financial, rules);
  const expenses = toRange(profile.financial.essentialMonthlyExpenses);
  const existing = toRange(profile.financial.existingEmis);
  if (!income || !expenses || !existing || safeEmi === null) return { passes: null, stressedMonthlySurplus: null, scenarios: [], reasons: [reason("STRESS_INPUTS_UNKNOWN", "stress", "caution", "STRESS_TEST", "Stress affordability needs income, expenses, existing EMIs, and a safe EMI estimate.", ["financial"])] };
  ...
  const scenarios: ("income" | "rate" | "expenses")[] = request.rateType === "floating" ? ["income", "rate", "expenses"] : ["income", "expenses"];
...
function selectedTenure(request: LoanRequest, product: ProductType, rules: EngineRules): number {
  const requested = toRange(request.preferredTenureMonths);
  const available = rules.tenureOptions.value[product];
  if (requested) return available.reduce((closest, candidate) => Math.abs(candidate - requested.low) < Math.abs(closest - requested.low) ? candidate : closest, available[0]!);
  return available[available.length - 1]!;
}

function confidence(profile: BorrowerProfile, offer: LoanOfferInputs | undefined, rules: EngineRules): ConfidenceLevel {
  let score = 100;
  const penalties = rules.confidencePenalties.value;
  if (profile.financial.monthlyIncome.kind === "unknown") score -= penalties.unknownIncome!;
  if (profile.financial.essentialMonthlyExpenses.kind === "unknown") score -= penalties.unknownExpenses!;
  if (profile.financial.existingEmis.kind === "unknown") score -= penalties.unknownExistingEmis!;
  if (profile.credit.scoreStatus !== "known") score -= penalties.unknownCredit!;
  if (!offer || offer.processingFee.kind === "unknown") score -= penalties.unknownFees!;
  if (profile.collateral.type !== "none" && (profile.collateral.ownershipVerified !== true || profile.collateral.unencumbered !== true)) score -= penalties.unverifiedCollateral!;
  if (profile.incomeStability !== "stable") score -= penalties.variableIncome!;
...
function verdict(
  profile: BorrowerProfile,
  request: LoanRequest,
  affordability: AssessmentResult["affordability"],
  stress: StressResult
): { value: BorrowingVerdict; reasons: ExplanationMetadata[] } {
  ...
  if (severeDebtStress) return { value: "dontBorrow", reasons: [reason("VERDICT_DONT_BORROW", "verdict", "blocking", "VERDICT_GATES", "Do not add new borrowing now because a recent repayment problem and high-cost debt indicate acute repayment stress.", ["financial", "credit.recentBounce", "credit.highCostDebtPresent"])] };
  if (!requested || !affordability.safeBorrowing || affordability.recommendedMaximumEmi === null) return { value: "needsInformation", reasons: [reason("VERDICT_NEEDS_INFORMATION", "verdict", "blocking", "VERDICT_GATES", "We need complete income, expense, EMI, and requested-amount information before making a borrowing recommendation.", ["financial", "request"])] };
  if (affordability.recommendedMaximumEmi === 0 || stress.passes === false) return { value: "dontBorrow", reasons: [reason("VERDICT_DONT_BORROW", "verdict", "blocking", "VERDICT_GATES", "Do not add new borrowing now because the conservative affordability or stress test fails.", ["financial", "credit.recentBounce", "credit.highCostDebtPresent"])] };
  if (requested.high > affordability.safeBorrowing.low) return { value: "borrowLess", reasons: [reason("VERDICT_BORROW_LESS", "verdict", "caution", "VERDICT_GATES", "The requested amount exceeds the conservative safe borrowing amount; reduce the amount or extend the assessment with more information.", ["request.requestedAmount", "affordability.safeBorrowing"])] };
...
function tenureTradeoffs(affordability: AssessmentResult["affordability"], request: LoanRequest, rate: RateBand, product: ProductType, stress: StressResult, rules: EngineRules): readonly TenureOption[] {
  if (!affordability.safeBorrowing || affordability.recommendedMaximumEmi === null) return [];
...
export function assessBorrower(input: AssessmentInput, rules: EngineRules = DEFAULT_RULES): AssessmentResult {
