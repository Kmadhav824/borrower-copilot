...
export function calculateEmi(principal: number, annualRate: number, months: number): number {
  if (principal < 0 || annualRate < 0 || months <= 0) throw new Error("Invalid loan inputs");
...
export function principalFromEmi(emi: number, annualRate: number, months: number): number {
  if (emi < 0 || annualRate < 0 || months <= 0) throw new Error("Invalid loan inputs");
...
export function monthlyIrr(cashflows: readonly number[]): number | null {
  if (cashflows.length < 2 || cashflows[0] === undefined || cashflows[0] <= 0 || !cashflows.slice(1).some((cashflow) => cashflow < 0)) return null;
  const npv = (rate: number): number => cashflows.reduce((sum, cashflow, index) => sum + cashflow / ((1 + rate) ** index), 0);
  let low = -0.999;
  let high = 1;
...
export function annualiseMonthlyRate(monthlyRate: number): number {
